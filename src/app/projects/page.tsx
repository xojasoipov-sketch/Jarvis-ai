...
const INIT: Project[] = [
  { id: 1, name: "Pari AI Platform", desc: "Ko'p agentli AI yordamchi tizim", status: "active", progress: 68, tasks: 24, done: 16, color: "from-indigo-500 to-purple-600", icon: "🤖", tags: ["Next.js", "AI", "SaaS"], updated: "Bugun" },
  { id: 2, name: "E-commerce Bot", desc: "Telegram orqali savdo avtomatlashtirish", status: "active", progress: 40, tasks: 18, done: 7, color: "from-green-500 to-teal-600", icon: "🛒", tags: ["Telegram", "Python", "Bot"], updated: "Kecha" },
  { id: 3, name: "Analytics Dashboard", desc: "Real-time biznes ko'rsatkichlari", status: "paused", progress: 25, tasks: 12, done: 3, color: "from-orange-500 to-red-600", icon: "📊", tags: ["React", "Charts", "SQL"], updated: "3 kun oldin" },
  { id: 4, name: "CRM Tizimi", desc: "Mijozlarni boshqarish platformasi", status: "done", progress: 100, tasks: 30, done: 30, color: "from-blue-500 to-cyan-600", icon: "👥", tags: ["CRM", "SaaS", "B2B"], updated: "1 hafta oldin" },
];
  ...
  return (
    <div className="fade-in max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-gray-100 rounded-xl p-1">
            <button onClick={() => setView("grid")} className={`px-3 py-1.5 text-xs rounded-lg transition-all ${view === "grid" ? "bg-white shadow text-gray-900" : "text-gray-500"}`}>⊞ Grid</button>
            <button onClick={() => setView("list")} className={`px-3 py-1.5 text-xs rounded-lg transition-all ${view === "list" ? "bg-white shadow text-gray-900" : "text-gray-500"}`}>☰ List</button>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Faol loyihalar", value: projects.filter(p => p.status === "active").length, icon: "🚀", color: "text-green-600" },
          { label: "Umumiy vazifalar", value: projects.reduce((a, p) => a + p.tasks, 0), icon: "✓", color: "text-indigo-600" },
          { label: "Yakunlangan", value: projects.reduce((a, p) => a + p.done, 0), icon: "🏆", color: "text-orange-600" },
        ].map(s => (
            ...
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:border-gray-200 transition-all">
              ...
              <div className="p-5">
                <div className="flex items-start gap-3 mb-4">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${p.color} flex items-center justify-center text-xl flex-shrink-0`}>{p.icon}</div>
        ...
        <div className="space-y-2">
          {projects.map(p => (
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-4 hover:border-gray-200 transition-all">
              <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${p.color} flex items-center justify-center text-lg flex-shrink-0`}>{p.icon}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
