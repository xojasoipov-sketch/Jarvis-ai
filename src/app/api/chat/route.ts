...
import { buildBrainContext, formatConnectionsReport } from "@/lib/connections";
...
Faqat system dagi HAQIQIY TIZIM HOLATI va tool lar ro'yxatidan foydalan.
O'zing tool inventar qilma (list_service_orders kabi nomlar o'ylab topma).
O'zbek savol → o'zbek javob. Internet kerak → web_search.`;
...
function isInventoryQuestion(text: string): boolean {
  return /nima ulan|nima connect|qanday tool|qaysi tool|ulangan|connected|connections|list_connections|nima bor|qanday funksiya|asosiy tool|vositalar|integratsiya/i.test(
    text
  );
}
...
async function autoContext(userText: string): Promise<string> {
  ...
  if (urlM) {
    try {
      if (/email/i.test(t)) {
        ...
        parts.push(JSON.stringify(await runAnyTool("extract_social_links", { url: urlM[0] })).slice(0, 3000));
      } else {
  ...
  } else if (/internet|qidir|search|yangilik|hozir|bugun|nima bo/i.test(userText)) {
    try {
...
export async function POST(req: NextRequest) {
  ...
  // ── SHORT-CIRCUIT: inventory questions → no LLM invention ──
  if (isInventoryQuestion(lastText)) {
    log("info", "chat-api", "inventory short-circuit");
    return textStream(formatConnectionsReport());
  }
  ...
  if (auto) return textStream("Natija:\n" + auto);
