    ...
    <div className="fade-in max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{files.length} ta fayl (Supabase Storage) — AI bilan tahlil qilish mumkin</p>
        </div>
      ...
      {!configured && (
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-sm text-amber-800">
          Supabase ulanmagan — fayllar saqlanmaydi. Connectors sahifasidan sozlang.
        </div>
      )}
          ...
          {selected ? (
            <>
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                {content ? (
                  <pre className="text-xs text-gray-700 whitespace-pre-wrap font-mono bg-gray-50 rounded-xl p-4 max-h-40 overflow-y-auto">{content}</pre>
                ) : (
                  <p className="text-sm text-gray-400 text-center py-6">Bu fayl turi uchun matn ko'rinishi yo'q</p>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-dashed border-gray-200 h-64 flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-indigo-300 transition-all"
              ...
              <p className="text-sm text-gray-500">Fayl tanlang yoki yuklang</p>
