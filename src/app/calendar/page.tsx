...
const COLORS = { meeting: "bg-blue-500", task: "bg-[#ff6a1a]", reminder: "bg-yellow-500", agent: "bg-purple-500" };
  ...
  return (
    <div className="fade-in max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#f5f1ea]">Calendar</h1>
          <p className="text-sm text-[#7d7870] mt-0.5">{MONTH_NAMES[month]} {year}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-2 bg-[#141316] rounded-2xl border border-white/[0.08] shadow-sm p-5">
          ...
          <div className="grid grid-cols-7 mb-2">
            {DAY_NAMES.map(d => (
              <div key={d} className="text-xs text-[#5c584f] font-medium text-center py-1">{d}</div>
            ))}
          </div>
              ...
              return (
                <button key={day} onClick={() => setSelectedDay(day)}
                  className={`relative aspect-square rounded-xl text-sm font-medium transition-all flex flex-col items-center justify-center gap-0.5
                    ${isSelected ? "bg-[#ff6a1a] text-white" : isToday ? "bg-[#ff6a1a]/10 text-[#ff8a3d]" : "hover:bg-[#0a0a0c] text-[#cfc9bd]"}`}>
                  {day}
                  {hasEvents && (
                    <span className={`w-1 h-1 rounded-full ${isSelected ? "bg-[#141316]" : "bg-[#ff6a1a]"}`} />
                  )}
                </button>
              );
            })}
          ...
          {/* Legend */}
          <div className="flex gap-4 mt-4 pt-3 border-t border-white/[0.08]">
            {Object.entries(TYPE_LABELS).map(([type, label]) => (
              <div key={type} className="flex items-center gap-1.5">
                <span className={`w-2 h-2 rounded-full ${COLORS[type as Event["type"]]}`} />
                <span className="text-xs text-[#7d7870]">{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="bg-[#141316] rounded-2xl border border-white/[0.08] shadow-sm p-4">
            <p className="text-sm font-semibold text-[#f5f1ea] mb-1">
              {selectedDay === TODAY.getDate() ? "Bugun" : `${selectedDay}-${MONTH_NAMES[month].slice(0, 3)}`}
            </p>
            <p className="text-xs text-[#7d7870]">{dayEvents.length} ta voqea</p>
          </div>

          {showNew && (
            <div className="bg-[#141316] rounded-2xl border border-[#ff6a1a]/30 p-4 space-y-2">
              <input value={newEvent.title} onChange={e => setNewEvent(p => ({ ...p, title: e.target.value }))}
                placeholder="Voqea nomi..." className="w-full px-3 py-2 bg-[#0a0a0c] border border-white/[0.12] rounded-lg text-sm focus:outline-none" />
              <div className="flex gap-2">
                <input type="time" value={newEvent.time} onChange={e => setNewEvent(p => ({ ...p, time: e.target.value }))}
                  className="flex-1 px-3 py-2 bg-[#0a0a0c] border border-white/[0.12] rounded-lg text-sm focus:outline-none" />
                <select value={newEvent.type} onChange={e => setNewEvent(p => ({ ...p, type: e.target.value as Event["type"] }))}
                  className="flex-1 px-2 py-2 bg-[#0a0a0c] border border-white/[0.12] rounded-lg text-xs focus:outline-none">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={addEvent} className="flex-1 py-2 bg-[#ff6a1a] text-white text-xs rounded-lg">Qo'shish</button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            {dayEvents.length === 0 ? (
              <div className="bg-[#141316] rounded-2xl border border-dashed border-white/[0.12] p-6 text-center">
                <p className="text-[#5c584f] text-sm">Voqealar yo'q</p>
              </div>
            ) : (
              dayEvents.map(ev => (
                <div key={ev.id} className="bg-[#141316] rounded-xl border border-white/[0.08] p-3 flex items-center gap-3">
                  <div className={`w-1 h-10 rounded-full ${ev.color} flex-shrink-0`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#f5f1ea] truncate">{ev.title}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-[#7d7870] flex items-center gap-1"><Clock size={11} strokeWidth={1.75} /> {ev.time}</span>
                      <span className="text-xs text-[#5c584f]">{TYPE_LABELS[ev.type]}</span>
                    </div>
                  </div>
                  <button onClick={() => setEvents(p => p.filter(e => e.id !== ev.id))}
                    className="p-1 text-[#454239] hover:text-red-400 transition-all"><X size={13} strokeWidth={1.75} /></button>
                </div>
              ))
            )}
          </div>
