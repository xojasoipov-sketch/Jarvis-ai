...
const COLORS = { meeting: "bg-blue-500", task: "bg-indigo-500", reminder: "bg-yellow-500", agent: "bg-purple-500" };
              ...
              dayEvents.map(ev => (
                <div key={ev.id} className="bg-white rounded-xl border border-gray-100 p-3 flex items-center gap-3">
                  ...
                  <button onClick={() => setEvents(p => p.filter(e => e.id !== ev.id))}
                    className="p-1 text-gray-300 hover:text-red-400 transition-all text-xs">✕</button>
                </div>
              ))
            )}
          </div>
