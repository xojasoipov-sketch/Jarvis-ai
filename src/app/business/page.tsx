...
import {
  Sparkles, RefreshCw, ChevronDown, ChevronUp, Trash2, Send,
  Play, Pause, Lightbulb, TrendingUp,
} from "lucide-react";
...
function ModulePanel({ mod, onRefresh }: { mod: BusinessModule; onRefresh: () => void }) {
  ...
  async function setStatus(status: ModuleStatus) {
    await fetch("/api/business", {
      ...
      body: JSON.stringify({ module_key: mod.module_key, status }),
    });
    onRefresh();
  }
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/business/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ module_key: mod.module_key, topic }),
      });
      const data = await res.json();
      if (data.idea) {
  ...
  async function sendToSmm(idea: Idea) {
    ...
    if (channels.length === 0) {
      alert("Avval SMM bo'limida kanal ulang.");
      return;
    }
    await fetch("/api/smm/posts", {
      ...
      body: JSON.stringify({
        ...
        content: idea.content.slice(0, 1000),
        topic: idea.title,
        status: "draft",
      }),
    });
    await updateIdeaStatus(idea.id, "used");
    alert("SMM postlar bo'limiga draft sifatida qo'shildi.");
  }

  async function updateIdeaStatus(id: string, status: Idea["status"]) {
    await fetch("/api/business/ideas", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${STATUS_COLOR[mod.status]}`}>
        ...
        <div className="flex items-center gap-2 mt-4 flex-wrap">
          {mod.status !== "active" && (
            ...
            >
              <Play size={12} strokeWidth={2} /> Faollashtirish
            </button>
          )}
          {mod.status === "active" && (
            <button
            ...
            >
              <Pause size={12} strokeWidth={2} /> To'xtatish
            </button>
          )}
          <button
        ...
        <div className="border-t border-gray-100 p-5 bg-gray-50/50 space-y-4">
          <div className="flex gap-2">
            <input
              ...
              placeholder="Mavzu yozing (masalan: motivatsiya, startap maslahatlar...)"
              className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:border-indigo-400"
            />
            <button
              onClick={generate}
              disabled={generating || !topic.trim()}
              className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 transition-colors flex-shrink-0"
            >
              <Sparkles size={13} strokeWidth={2} className={generating ? "animate-pulse" : ""} />
              {generating ? "Yaratyapman..." : "AI bilan yaratish"}
            </button>
          </div>
          ...
          <div className="space-y-2">
            {ideas.length === 0 && !loadingIdeas && (
              <p className="text-xs text-gray-400 text-center py-4">Hali g'oya yaratilmagan.</p>
            )}
            {ideas.map((idea) => (
              <div key={idea.id} className="bg-white border border-gray-100 rounded-xl p-3">
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {idea.status === "used" && (
                      <span className="px-1.5 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] rounded-full">SMM'ga yuborilgan</span>
                    )}
                    <button
                      onClick={() => sendToSmm(idea)}
                      title="SMM postiga aylantirish"
    ...
    <div className="fade-in max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">AI yordamida daromad keltiradigan yo'nalishlarni boshqarish va rivojlantirish</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 gap-3">
        <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-emerald-50 flex items-center justify-center flex-shrink-0">
            <Play size={15} strokeWidth={1.75} className="text-emerald-600" />
          </div>
          <div>
            <p className="text-xs text-gray-400">Faol modullar</p>
            <p className="text-lg font-bold text-gray-900">{activeCount} / {modules.length || 5}</p>
          </div>
        </div>
        <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-indigo-50 flex items-center justify-center flex-shrink-0">
            <TrendingUp size={15} strokeWidth={1.75} className="text-indigo-600" />
          </div>
          <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {modules.map((mod) => (
          <ModulePanel key={mod.module_key} mod={mod} onRefresh={load} />
        ))}
      </div>
    </div>
  );
