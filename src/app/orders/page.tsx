...
import { ShoppingCart, Plus, Search, Filter, Play, Clock, CheckCircle2, XCircle, AlertCircle, User, Calendar, DollarSign } from "lucide-react";
            ...
            return (
              <div key={o.id} className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-4 hover:border-gray-200 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-1">
                    <span className="flex items-center gap-1 text-xs text-gray-400">
                      <User size={11} /> {o.client_name || "Noma'lum"}
                    </span>
                    {o.amount ? (
                      <span className="flex items-center gap-1 text-xs text-gray-400">
                        <DollarSign size={11} /> {o.amount.toLocaleString()} {o.currency || "UZS"}
                      </span>
                    ) : null}
                    <span className="flex items-center gap-1 text-xs text-gray-400">
                      <Calendar size={11} /> {new Date(o.created_at).toLocaleDateString("uz-UZ")}
                    </span>
                  </div>
                </div>
                {o.status !== "done" && o.status !== "cancelled" && (
                  <button
                    onClick={() => startWork(o)}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg hover:bg-gray-800 transition-colors flex-shrink-0"
                  >
                    <Play size={11} />
                    Ishlash
                  </button>
