...
const FILTER_MAP: Record<string, string | null> = {
  "Barchasi": null, "Yangi": "new", "Jarayonda": "in_progress", "Tekshiruv": "review", "Tayyor": "done", "Bekor": "cancelled",
};
...
export default function OrdersPage() {
  ...
  const filtered = orders.filter(o => {
    ...
    const searchMatch = !search || o.client_name?.toLowerCase().includes(search.toLowerCase()) || o.service_name?.toLowerCase().includes(search.toLowerCase());
  ...
  const stats = {
    ...
    revenue: orders.filter(o => o.status === "done").reduce((s, o) => s + (o.amount || 0), 0),
  };

  function startWork(o: Order) {
    router.push(`/chat?q=${encodeURIComponent(`${o.service_name} — mijoz: ${o.client_name}. Buyurtma #${o.id}. Ishni davom ettiraylik.`)}`);
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => router.push("/services")} className="flex items-center gap-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg hover:bg-gray-800">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Daromad", value: stats.revenue ? `$${stats.revenue.toLocaleString()}` : "—", sub: "to'langan" },
        ].map(s => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Mijoz yoki xizmat..."
        ...
        <div className="flex gap-1 flex-wrap">
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 text-xs rounded-lg ${filter === f ? "bg-gray-900 text-white" : "text-gray-500 hover:bg-gray-100"}`}>{f}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map(o => {
            ...
            const SIcon = sc.icon;
            return (
              <div key={o.id} className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-4 hover:border-gray-200">
                <div className={`p-2 rounded-lg ${sc.bg} flex-shrink-0`}><SIcon size={16} className={sc.color} /></div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-1 flex-wrap">
                    ...
                    {o.amount ? <span className="flex items-center gap-1 text-xs text-gray-400"><DollarSign size={11} /> {o.amount.toLocaleString()} {o.currency || "UZS"}</span> : null}
                ...
                {o.status !== "done" && o.status !== "cancelled" && (
                  <button onClick={() => startWork(o)} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 text-white text-xs rounded-lg hover:bg-gray-800 flex-shrink-0">
